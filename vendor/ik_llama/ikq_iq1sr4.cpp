// Vendored IQ1_S_R4 block codec from ik_llama.cpp.
//
...
//   ggml/src/ggml-common.h    block_iq1_s_r4, IQ1S_DELTA
//   ggml/src/ggml-quants.c    kgrid_1bit_2048, the IQ1_S branch of
//                             iq2xs_init_impl, iq2_compare_func,
//                             iq1_sort_helper, iq1_find_best_neighbour2,
//                             iq1s_process_1block, GROUP_MAX_EPS_IQ1_S
//   ggml/src/iqk/iqk_quantize.cpp
//                             nearest_int, quantize_iq1_s_r4,
//                             dequantize_row_iq1_s_r4
//   ggml/src/ggml-impl.h      the NEON fp16 conversion pair
//
// The copied bodies are byte-for-byte the upstream ones. What changed, and
// only this, is the surrounding scaffolding: the ggml type-trait lookup
// becomes the literal row-size expression, GGML_ASSERT becomes a return of
// zero so a rejected geometry surfaces to the caller instead of aborting a
// long conversion, and the grid, map, and neighbour tables that upstream
// builds in ggml_quantize_init are built here on first use from the same
// kgrid_1bit_2048 codes by the same loops.
//
// Upstream's dequantizer reads a static 2048-entry uint64 value table
// (iq1s_grid in ggml-common.h). That table is derivable from the packed
// codes: byte i of entry k is ((kgrid_1bit_2048[k] >> 2*i) & 3) - 1, and the
// derivation reproduces the shipped table entry for entry. The decode table
...
// ggml-quants.c, the packed 1-bit grid codes. Each of the 2048 entries packs
// eight ternary positions at two bits per position.
...
// The runtime tables upstream builds in iq2xs_init_impl for GGML_TYPE_IQ1_S,
// built here on first use by the same loops: the quantizer grid holds
// position bytes 2*l + 1, the decode grid holds the values l - 1, the map
// sends an 8-position code word to its grid index or to the start of its
// neighbour run, and the neighbour lists carry the nwant = 3 nearest
// distance shells.
struct Iq1sTables {
    std::vector<uint64_t> grid;        // quantizer grid, bytes 2*l + 1
    std::vector<uint64_t> decode;      // decode grid, bytes l - 1
    std::vector<int>      map;
    std::vector<uint16_t> neighbours;
};

// ggml-quants.c iq2_compare_func
int iq2_compare_func(const void * left, const void * right) {
    const int * l = (const int *)left;
    const int * r = (const int *)right;
    return l[0] < r[0] ? -1 : l[0] > r[0] ? 1 : l[1] < r[1] ? -1 : l[1] > r[1] ? 1 : 0;
}

// ggml-quants.c iq1_sort_helper
...
const Iq1sTables & iq1s_tables() {
    static Iq1sTables t = [] {
        Iq1sTables out;
        const int grid_size = NGRID_IQ1S;
        const int kmap_size = 43692;
        const int nwant = 3;

        out.grid.resize(grid_size);
        out.decode.resize(grid_size);
        for (int k = 0; k < grid_size; ++k) {
            int8_t * pos = (int8_t *)(out.grid.data() + k);
            int8_t * val = (int8_t *)(out.decode.data() + k);
            for (int i = 0; i < 8; ++i) {
                int l = (kgrid_1bit_2048[k] >> 2*i) & 0x3;
                pos[i] = (int8_t)(2*l + 1);
                val[i] = (int8_t)(l - 1);
            }
        }
        out.map.assign(kmap_size, -1);
        uint64_t aux64;
        ...
        int8_t pos[8];
        std::vector<int> dist2(2*grid_size);
        int num_neighbors = 0, num_not_in_map = 0;
        for (int i = 0; i < kmap_size; ++i) {
            if (out.map[i] >= 0) continue;
            ++num_not_in_map;
            for (int k = 0; k < 8; ++k) {
                int l = (i >> 2*k) & 0x3;
                pos[k] = (int8_t)(2*l + 1);
            ...
            for (int j = 0; j < grid_size; ++j) {
                const int8_t * pg = (const int8_t *)(out.grid.data() + j);
                int d2 = 0;
                for (int k = 0; k < 8; ++k) d2 += (pg[k] - pos[k])*(pg[k] - pos[k]);
                dist2[2*j+0] = d2;
                dist2[2*j+1] = j;
            }
            qsort(dist2.data(), grid_size, 2*sizeof(int), iq2_compare_func);
        ...
        out.neighbours.resize(num_neighbors + num_not_in_map);
        int counter = 0;
        for (int i = 0; i < kmap_size; ++i) {
            if (out.map[i] >= 0) continue;
            for (int k = 0; k < 8; ++k) {
            ...
            out.map[i] = -(counter + 1);
            int d2 = dist2[0];
            uint16_t * start = &out.neighbours[counter++];
            int n = 0, nhave = 1;
            for (int j = 0; j < grid_size; ++j) {
                if (dist2[2*j] > d2) {
                    if (nhave == nwant) break;
                ...
                out.neighbours[counter++] = (uint16_t)dist2[2*j+1];
                ++n;
            }
            *start = (uint16_t)n;
        }
...
// ggml-quants.c iq1_find_best_neighbour2
int iq1_find_best_neighbour2(const uint16_t * neighbours, const uint64_t * grid,
        const float * xval, const float * weight, float scale, const float * xg,
        int8_t * L, int ngrid) {
    ...
    if (grid_index < 0) return -1;
    const int8_t * pg = (const int8_t *)(grid + grid_index);
    for (int i = 0; i < 8; ++i) L[i] = (pg[i] - 1)/2;
    return grid_index;
}

// ggml-quants.c iq1s_process_1block. The iq2_data lookup becomes the local
// table struct; the asserts on a missing init cannot fire (the tables build
// on first use) and the geometry asserts become error returns in the caller.
int iq1s_process_1block(int block_size, const float * xb, const float * weight,
        int8_t * L, float * the_scale, uint16_t * the_index, int * the_shift,
        float * pairs, float * sumx, float * sumw) {
    float max = fabsf(xb[0]);
    for (int i = 1; i < block_size; ++i) max = fmaxf(max, fabsf(xb[i]));
    ...
    const Iq1sTables & tables = iq1s_tables();
    const uint64_t * kgrid_q2xs      = tables.grid.data();
    const int      * kmap_q2xs       = tables.map.data();
    const uint16_t * kneighbors_q2xs = tables.neighbours.data();
    ...
    if (besti1 < 0 || besti2 < 0 || best_shift == 0) return 1;
...
size_t row_size_iq1_s_r4(int64_t n_per_row) {
    if (n_per_row <= 0 || n_per_row % 32) return 0;
    return 2 + 6 * (size_t)(n_per_row / 32);
}

// iqk_quantize.cpp quantize_iq1_s_r4. The GGML_ASSERTs on geometry become
// the zero-return contract of the wrapper; ggml_row_size becomes the literal
// expression; the imatrix mismatch fallback keeps upstream's printf so a
// fallback block is as loud here as it is there.
size_t quantize_iq1_s_r4_impl(const float * src, void * dst, int64_t nrows,
    ...
    for (int row = 0; row < nrows; row += 4) {
        ...
        for (int ibl = 0; ibl < nblock; ++ibl) {
            for (int k = 0; k < 4; ++k) {
                ...
                if (iq1s_process_1block(kBlockSize, xb, weight, L,
                                        scales.data() + 4*ibl + k, index, &shift,
                                        pairs, sumx, sumw)) {
                    return 0;
                }
                if (scales[4*ibl+k] < 0) return 0;
                max[k] = std::max(max[k], scales[4*ibl+k]);
                uint16_t h = 0;
                for (int i = 0; i < 4; ++i) {
                    if (index[i] >= 2048) return 0;
...
// iqk_quantize.cpp dequantize_row_iq1_s_r4, one four-row group per call.
...
int ikq_dequantize_iq1_s_r4(const void * src, float * dst, int64_t nrows,
    ...
    if (!row_size) return 1;
    const char * cx = (const char *)src;
    for (int64_t row = 0; row < nrows; row += 4) {
        dequantize_group_iq1_s_r4(
            (const block_iq1_s_r4 *)(cx + row * row_size),
            dst + row * n_per_row, 4 * n_per_row);
    }
    return 0;
}
